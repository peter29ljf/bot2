 ...
 */
async function openLong(req, res) {
  try {
    ...
    // 如果所有精度都失败
    console.log('所有精度都失败，尝试不带止盈止损的简单订单');
    ...
    try {
      console.log('简单订单数据:', JSON.stringify(simpleOrderData, null, 2));
      const response = await bitgetClient.placeOrder(simpleOrderData);
      console.log('简单订单API响应:', JSON.stringify(response, null, 2));
